...
void httpServer_run(uint8_t seqnum)
{
	...
	switch(getSn_SR(s))
	{
		case SOCK_ESTABLISHED:
		{
			...
			// Der ursprüngliche code wurde in "HttpServer_ProcessRequest" verschoben. 
			// Dadurch kann eine eigene TCPLoop verwendet werden 
			HttpServer_ProcessRequest(seqnum);
...
void HttpServer_ProcessRequest(uint8_t seqnum )
{
	uint8_t s;	// socket number
	uint16_t len;
	uint32_t gettime = 0;
